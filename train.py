...
class GDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        obj = self.data[idx]
        token_ids = [int(pair.split(":")[1]) for pair in obj]  # Extract numeric values
        input_seq = torch.tensor(token_ids[:-1], dtype=torch.long)  # All except last token
        target = torch.tensor(token_ids[-1], dtype=torch.long)  # Last token as target
